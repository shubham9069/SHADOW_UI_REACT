...
const ShadowCheckbox = ({ checked = null, label = "", disabled = false, styleType = "default", type = "default", onClick }: ShadowCheckboxProps) => {
  ...
  return (
    <div className="shadow-checkbox-container">
      {type === "triState" && <TriStateCheckbox className={styleType} disabled={disabled} value={value} onChange={(e) => onCheckboxClick(e.value)} />}
      {type === "default" && (
        <Checkbox className={styleType} disabled={disabled} checked={value} onChange={(e) => onCheckboxClick(e.checked)}></Checkbox>
      )}
