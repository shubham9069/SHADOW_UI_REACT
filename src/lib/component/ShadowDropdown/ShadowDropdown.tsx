...
const ShadowDropdown = ({
  ...
  const [selectedDropdown, setSelectedDropdown] = useState<ShadowDropdownOption | ShadowDropdownGroupOption | null>(selectedOption?.value);
  ...
  const selectedOptionTemplate = (option: ShadowDropdownOption | ShadowDropdownGroupOption | null, props: any) => {
    if (selectedOption) {
      return (
  ...
  const optionTemplate = (option: ShadowDropdownOption | ShadowDropdownGroupOption | null) => {
    if (option) {
      return (
        <div className="withIcons-options" key={option.value}>
          {option.icon &&  <i className={option.icon}></i>}
    ...
    <div className="shadow-dropdown-container">
      {label && <span className="label-text">{label}</span>}
        <Dropdown
          ...
          disabled={disabled}
          filter={filter}
          {...(virtualScroll && { virtualScrollOptions: { itemSize: virtualScrollItemSize } })}
          editable={editable}
