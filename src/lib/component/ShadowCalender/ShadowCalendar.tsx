...
const ShadowCalendar = ({
  ...
  const checkDate = (date: any) => {
    ...
    if (Array.isArray(value)) {
      ...
      returnFlag = value.getDate() === date.day && value.getMonth() === date.month && value.getFullYear() === date.year;
    }
    return returnFlag;
  };
      ...
      case "multiple":
        if (Array.isArray(newValue)) {
          const index = newValue.findIndex((d) => {
            return d.getFullYear() === input.year && d.getDate() === input.date && d.getMonth() === input.month;
          });
          if (index > -1) {
  ...
  const getMonthClass = (month: number) => {
    ...
    switch (selectionMode) {
      case "range":
        if (Array.isArray(value)) {
          if (value[0]) {
            cls += value[0].getFullYear() === activeYear && value[0].getMonth() === month ? " calendar-selected-custom-item" : "";
          }
          if (value[1]) {
            if (value[1].getMonth() === month && value[1].getFullYear() === activeYear) {
              cls += " calendar-selected-custom-item";
            } else if (date.valueOf() > value[0].valueOf() && date.valueOf() < value[1].valueOf()) {
              cls += " calendar-highlight-custom-item";
            }
          }
        }
      ...
      case "single":
        if (value instanceof Date) {
          cls += value.getFullYear() === activeYear && value.getMonth() === month ? " calendar-selected-custom-item" : "";
        }
        break;
    }
  ...
  const getDateClass = (date: any) => {
    ...
    if (today.getFullYear() === year && today.getDate() === date.date && today.getMonth() === month) {
      cls += " calendar-highlight-custom-item";
    }

    if (minDate && maxDate && (date.dateItem.valueOf() < minDate.valueOf() || date.dateItem.valueOf() > maxDate.valueOf())) {
      cls += " calendar-disabled-date";
    }

    if (disabledDates.length) {
      const idx = disabledDates.findIndex((d) => {
        return d.getFullYear() === year && d.getMonth() === month && d.getDate() === date.date;
      });
      if (idx !== -1) {
        cls += " calendar-disabled-date";
      }
    ...
    switch (selectionMode) {
      case "range":
        if (Array.isArray(value)) {
          if (value[0]) {
            cls +=
              value[0].getFullYear() === year && value[0].getMonth() === month && value[0].getDate() === date.date
                ? " calendar-selected-custom-item"
                : "";
          }
          if (value[1]) {
            if (value[1].getMonth() === month && value[1].getFullYear() === year && value[1].getDate() === date.date) {
              cls += " calendar-selected-custom-item";
            } else if (date.dateItem.valueOf() >= value[0].valueOf() && date.dateItem.valueOf() <= value[1].valueOf()) {
              cls += " calendar-highlight-custom-item";
            }
          }
        }
      ...
      case "single":
        if (value instanceof Date) {
          cls += value.getFullYear() === year && value.getMonth() === month && value.getDate() === date.date ? " calendar-selected-custom-item" : "";
        }
        break;
    }
  ...
  const getYearClass = (year: number) => {
    ...
    switch (selectionMode) {
      case "range":
        if (Array.isArray(value)) {
          if (value[0]) {
            cls += value[0].getFullYear() === year ? " calendar-selected-custom-item" : "";
          }
          if (value[1]) {
            if (value[1].getFullYear() === year) {
              cls += " calendar-selected-custom-item";
            } else if (year > value[0].getFullYear() && year < value[1].getFullYear()) {
              cls += " calendar-highlight-custom-item";
            }
          }
        }
      ...
      default:
        if (value instanceof Date) {
          cls += value.getFullYear() === year ? " calendar-selected-custom-item" : "";
        }
        break;
    }
  ...
  const createMonthData = (activeYear: number, activeMonth: number) => {
    ...
    for (let j = 0; j < numberOfMonths; j++) {
      ...
      while (i !== -1) {
        ...
        let newWeek = false;
        if (day === 0) {
          let dayCount = Math.floor((date.valueOf() - startDate.valueOf()) / (24 * 60 * 60 * 1000));
          weekNumber = Math.ceil(dayCount / 7);
          newWeek = true;
        }

        if (dateNumber === i || day !== 0) {
          const monthValue = dateNumber !== i ? (month === 11 ? 0 : month + 1) : month;
          const yearValue = monthValue !== month && monthValue === 0 ? year + 1 : year;
          days.push({
            date: dateNumber,
      ...
      items.forEach(function (d) {
        ...
        switch (d) {
          ...
          case "m":
            ...
            break;
          case "mm":
            replacementString = (String(month + 1).length === 1 ? "0" : "") + month;
            break;
          case "M":
        ...
        resultStr = (dayText ? dayText + " " : "") + resultStr + replacementString + separator;
      });
      res.push(resultStr);
    });
  ...
  const inputField = (overlay: any) => (
    <span onClick={(e) => showHandler(overlay, e)} className="shadow-custom-calendar-input-container p-input-icon-right">
      <InputText type="text" defaultValue={valueStr} style={{ inputStyle } as Object} className={inputStyleClass} placeholder={placeholder} />
      ...
      <>
        ...
        <span className="shadow-calendar-header">
          <span className="shadow-calendar-titlebar">
            ...
            <span className="shadow-calendar-custom-label">
              <span className="shadow-calendar-title-item" onClick={onDecadeClick}>
        ...
        <div className="shadow-calendar-custom-body shadow-calendar-year-body">
          {createYearRange(10).map((year, idx) => (
            <span key={`${year}-${idx}`} onClick={() => onYearClick(year)} className={getYearClass(year)}>
              {year}
            </span>
          ))}
        </div>
      ...
      <>
        ...
        <div className="shadow-calendar-header">
          {activeMonths.map((month, i) => (
            <span className="shadow-calendar-titlebar" key={`activeMonth-${i}`}>
              ...
              <span className="shadow-calendar-custom-label">
                <span onClick={() => showMonths()} className="shadow-calendar-month-label shadow-calendar-title-item">
                  {monthLabels[activeMonth + i]}
                </span>
                <span onClick={() => showYears()} className="shadow-calendar-title-item">
                  {activeYear}
                </span>
              </span>
        ...
        <div className="shadow-calendar-custom-body shadow-calendar-date-body">
          {activeMonths.map((month, idx) => (
            <div className={`calendar-month-grid ${showWeek ? "calendar-show-week-column" : "hide-week-column"}`} key={`month-${idx}`}>
              ...
              {month?.map((day: any, idx: number) => (
                <React.Fragment key={`${day?.day}-${idx}`}>
                  {showWeek && !day?.day && <span className="calendar-week-number">{day?.weekNumber}</span>}
                  <span onClick={() => onDateClick(day)} className={getDateClass(day)}>
                    {day?.date}
                  </span>
                </React.Fragment>
              ))}
      ...
      <>
        ...
        <span className="shadow-calendar-header">
          <span className="shadow-calendar-titlebar">
            ...
            <i className="pi pi-chevron-right cal-change-icon" onClick={onMonthBracketUp}></i>
          </span>
        </span>
        <div className="shadow-calendar-custom-body shadow-calendar-month-body">
          {months.map((month, i) => (
            <span key={`month_cal-${i}`} onClick={() => onMonthClick(i)} className={getMonthClass(i)}>
              {month}
            </span>
          ))}
        </div>
    ...
    <React.Fragment>
      <div className={`card flex justify-content-center`}>
        {type === "date" && (
          <>
            {!inline ? (
              <div className={`shadow-calendar-content ${styleClass}${disabled ? " disabled-calendar" : ""}`} style={{ style } as Object}>
                <OverlayPanel
        ...
        {type === "year" && (
          <>
            <div className={`shadow-calendar-content ${styleClass}${disabled ? " disabled-calendar" : ""}`} style={{ style } as Object}>
              <OverlayPanel className={`shadow-custom-calendar ${panelStyleClass}`} onHide={hideHandler} style={panelStyle as any} ref={yearModal}>
                {yearCalendarContent()}
              </OverlayPanel>
