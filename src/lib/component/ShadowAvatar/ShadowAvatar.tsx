...
import { ShadowAvatarGroup, ShadowAvatarShape, ShadowAvatarSize, ShadowAvatarThemeStyle, ShadowAvatarType } from '../../models';
...
const ShadowAvatar: React.FC<ShadowAvatarProps> = ({
    size = 'normal',
    type = 'icon',
    grouping = false,
    themeStyle = 'neutral',
    ...
    const getAvatarObj = (type: ShadowAvatarType, avatar: any): ShadowAvatarGroup | undefined => {
        ...
        if (type === 'image' && avatar) {
            ...
            const avatarClass = avatar.querySelector('.p-avatar-icon')?.className || '';
            item = (avatarGroupData || []).find((itm) => itm.icon && avatarClass.includes(itm.icon));
        }
        return item;
    };
    ...
    const checkOverflow = () => {
        setTimeout(() => {
            const containerWidth = (el.current?.getBoundingClientRect().width || 0) - 35;
            ...
            if (avatars.length > 0) {
                ...
                for (const avatar of avatars) {
                    totalWidth += (avatar.getBoundingClientRect().width || 0) - (count > 0 ? 16 : 0);
                ...
                setOverflowAvatarLabel(newOverflowAvatars.length > 0 ? `+${newOverflowAvatars.length}` : '');
    ...
    return (
        ...
        <span className={`overflow-avatars-container inline-block`}>
          ...
          <Avatar
            ...
            {
                type == 'initials' && grouping == false && <Avatar
                    className={`${type} ${actionStyle} ${themeStyle} ${size}`}
                    label={initials || ''}
                    size={size}
                    shape={shape}
            ...
            {
                type == 'icon' && grouping == false && <Avatar
                    className={`${type} ${actionStyle} ${themeStyle} ${size}`}
                    icon={icon}
                    size={size}
                    shape={shape}
            ...
            {
                type == 'image' && grouping == false && <Avatar
                    image={imageUrl}
                    size={size}
                    shape={shape}
                ></Avatar>
