...
const ShadowChip = ({
    ...
    setTimeout(() => {
      ...
      if (overflowContainerRef.current) {
        const containerWidth =
          overflowContainerRef.current?.getBoundingClientRect().width - 25 // 25 is offset used for displaying numbers
        const chips: any =
          overflowContainerRef.current?.querySelectorAll('.shadow-chip')
        if (chips?.length) {
          let totalWidth = 0
  ...
  const getExtraClasses = (chip: ShadowChipItem) => {
    return `${chip.type || 'textOnly'} ${chip.avatarSize || 'small'} ${chip.theme ? chip.theme : themeStyle} ${
      chip.actionStyle ? chip.actionStyle : actionStyle
  ...
  return (
    <div
      ...
      className={`shadow-chips-container ${viewMode ? 'view-mode' : ''}`}
    >
      <div className='chips-box'>
        {(overflowEnabled ? visibleChips : chipList).map(
          (chip: any, index: any) => (
            <Chip
              key={`chip-1-${index}`}
            ...
            >
              {chip?.type === 'withIcon' && chip.icon && (
                <i className={`icon ${chip.icon}`} />
              )}
